...
const ProductById = ({ params }: { params: { id: string } }) => {
  ...
  const getData = async (id: string) => {
    ...
    setSubCatProduct(
      resp?.data?.Products.filter((el: { _id: string }) => el._id !== params.id)
    );
  };
  const products = async () => {
    try {
      ...
      if (res.status == 200 && res.message === "Product found successfully") {
        ...
        let productArray: any[] = storedProducts
          ? JSON.parse(storedProducts)
          : [];

        const productIndex = productArray.findIndex(
          (p: { productId: string }) => p.productId === productData.productId
        );
        if (productIndex > -1) {
  ...
  const handleAddToCart = async () => {
    ...
    const find = data.filter(
      (el: { id: string }) => el.id === productData?._id
    );
  ...
  return (
    <>
      <div className=" bg-white py-4 mt-7 mb-7 mr-12 ml-12 ">
            ...
            <div className="lg:col-gap-12 xl:col-gap-16 mt-8 grid grid-cols-1 gap-12 lg:mt-12 lg:grid-cols-5 lg:gap-16">
              ...
              <div className="lg:col-span-2 lg:row-span-2 lg:row-end-2">
                <h1 className="sm: text-2xl font-bold text-gray-900 sm:text-3xl">
                  {productData?.productBrand} - {productData?.productName}
                </h1>
                ...
                <div className="mt-6  gap-1">
                  <li className=" rounded-lg  py-2 font-bold">
                ...
                <div className="flex flex-col my-24 border border-gray-100 p-4">
                  ...
                  {review && review?.length > 0
                    ? review.map(
                        (
                          data: {
                            userId: { profileImg: string; fullName: string };
                            rating: number;
                            comment: string;
                          },
                          index: number
                            ...
                            <div className="ml-6">
                              <p className="flex items-baseline">
                                <span className="text-gray-600 font-bold">
                                  {data?.userId?.fullName[0].toUpperCase() +
                                    data?.userId?.fullName.slice(1)}
                                </span>
              ...
              <div className="lg:col-span-3">
                <div className="border-b border-gray-300">
                  <nav className="flex gap-4">
                    <p
                      title=""
                      className="border-b-2 border-gray-900 py-4 text-lg font-bold text-gray-900 hover:border-gray-400 hover:text-gray-800"
                    >
                      {" "}
                      Description{" "}
                    </p>
      ...
      <section className="px-2 bg-white py-4 mt-7 mb-7 mr-12 ml-12">
        ...
        <Swiper
          ...
          spaceBetween={10}
        >
          {subCatProduct && subCatProduct.length > 0
            ? subCatProduct.map(
                (
                  subval: {
                    _id: string;
